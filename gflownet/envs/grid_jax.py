...
class GridConfig:
    ...
    def _compute_action_space(self) -> List[Tuple[int, ...]]:
        ...
        actions.append(tuple([0 for _ in range(self.n_dim)]))  # EOS action
        return actions
...
def states2policy_jax(states: jnp.ndarray, config: GridConfig) -> jnp.ndarray:
    ...
    cols = states + offsets[None, :]
    
    rows = jnp.repeat(jnp.arange(batch_size), config.n_dim)
...
def states2proxy_jax(states: jnp.ndarray, config: GridConfig) -> jnp.ndarray:
    ...
    """
    states_onehot = states2policy_jax(states, config)
    states_onehot = states_onehot.reshape((states.shape[0], config.n_dim, config.length))
    
    cells = jnp.array(config.cells, dtype=jnp.float32)
    states_proxy = jnp.sum(states_onehot * cells[None, None, :], axis=2)
...
def get_mask_invalid_actions_forward_batch(
    ...
    invalid = jnp.any(children >= config.length, axis=-1)
...
def get_mask_invalid_actions_backward_batch(
    ...
    invalid = jnp.any(parents < 0, axis=-1)
    
    eos_mask = jnp.where(done, False, True)[:, None]
    mask = jnp.concatenate([invalid, eos_mask], axis=-1)
...
def step_batch(
    ...
    is_eos = jnp.all(actions == 0, axis=-1)
    
    at_max = jnp.all(states == config.length - 1, axis=-1)
...
def get_parents_batch(
    ...
    valid = jnp.all(parents >= 0, axis=-1)
...
class GridJAX:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.cells = np.array(self.config.cells)
        self.action_space = list(self.config.action_space)
    ...
    def plot_reward_samples(
        ...
        **kwargs,
        ...
        samples_hist, _, _ = np.histogram2d(
...
def sample_trajectories_jax(
    ...
    #! Initial state, necessary for jax.lax.scan
    ...
    def step_fn(carry, step_idx):
        ...
        logits = vmap(policy_model)(states_policy)
        ...
        # Execute environment step for the entire batch
    ...
    # Flatten trajectory data from [batch, time, ...] to [batch * time, ...] for training
